...
win = font1.render('YOU WIN!', True, (5, 255, 22))
lose = font1.render('YOU LOSE!', True, (180, 0, 0))
rel_time = False #flag in charge of reload
num_fire = 0 #variable to count shots

while run:
    for e in event.get(): #mengambil segala event(kejadian) selama game barlangsung
        if e.type == QUIT:
            run = False
        elif e.type == KEYDOWN:
            if e.key == K_SPACE:
    ...
    if finish != True:
        ...
        for c in collides:
            #this loop will repeat as many times as the number of monster of monster hit
            score = score + 1
            monster = Enemy('ufo.png', randint(80, win_width - 80), -40, 80, 50, randint(1, 5))
            monsters.add(monster)
